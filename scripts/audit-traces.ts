...
for(const name of names){
 const response=await fetch(`${base}/api/public/traces?name=${encodeURIComponent(name)}&limit=5`,{headers});if(!response.ok)throw new Error(`Trace API HTTP ${response.status}`);
 const data=await response.json();
 for(const found of data.data||[]){
  const resp=await fetch(`${base}/api/public/traces/${found.id}`,{headers});if(!resp.ok)throw new Error(`Trace API HTTP ${resp.status}`);
